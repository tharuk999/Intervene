...
export default function InterventionsChoiceScreen({ navigation }: any) {
      ...
      <ScrollView showsVerticalScrollIndicator={false} className="flex-1 px-5">
          ...
          {filtered.map(app => (
            <TouchableOpacity
              ...
              className={`bg-white rounded-2xl px-4 py-4 flex-row items-center ${
                !app.hasInterventions ? 'bg-[#F0EDE7]' : ''
              }`}
              onPress={() =>
                app.hasInterventions && navigation?.navigate('InterventionSetup', { app })
              }
            >
              {/* App Icon */}
              <View
              ...
              <View className="flex-1">
                <Text
                  className={`text-base font-semibold ${
                    app.hasInterventions ? 'text-[#1C1C1C]' : 'text-[#9A9A9A]'
                  }`}
                >
                  {app.name}
                </Text>
        ...
        <View className="bg-[#E8E5DF] rounded-3xl p-5 mt-6 mb-32 items-center">
          <Text className="text-[#6B6B6B] text-sm mb-3">
            Can't find an application in the list?
          </Text>
